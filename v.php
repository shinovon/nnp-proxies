...
function client_headers($ch, $headers) {
	...
	array_push($curlheaders, "User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:132.0) Gecko/20100101 Firefox/132.0");
...
$ins = $_GET['inv'] ?? file_get_contents('./invapicache') ?? 'https://pol1.iv.ggtyler.dev/';
$id = $_GET['v'];
$itag = $_GET['i'] ?? '18';
...
if (!$r || isset($r['error']) || !isset($r['formatStreams'])) {
	http_response_code(500);
	die;
}

foreach ($r['formatStreams'] as $f) {
	if ($f['itag'] == $itag || ($f['qualityLabel'] ?? '') == $itag) {
		$url = $f['url'];
		break;
	}
}
...
if (!$url) {
	foreach ($r['adaptiveFormats'] as $f) {
		if ($f['itag'] == $itag || ($f['qualityLabel'] ?? '') == $itag) {
			$url = $f['url'];
			break;
		}
...
$host = "https://{$host}";
$url = "/videoplayback?{$query_string}";
